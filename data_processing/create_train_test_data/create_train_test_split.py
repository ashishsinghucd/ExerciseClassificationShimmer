...
def train_test_split(segment_df_list, segment_label_list, segment_file_name_list, current_pids_list, gender_info_list, seed_value):
    """
    Function to split the pids in training, testing and validation data
    """
    train_pids, test_pids, train_gender, test_gender = split_pids(current_pids_list, gender_info_list,
                                                                        seed_value, train_test_split_ratio)
    val_pids = np.array([])
    if validation_data:
        train_pids, val_pids, train_gender, val_gender = split_pids(train_pids, gender_info_list,
                                                                          seed_value, split_val_ratio)
...
def create_final_formatted_data(seed_value):
    ...
    # First read all the clips with the information for peaks
    for clip_file in clip_files_list:
        try:
            ...
            # pid, exercise_type = clip_file[:-4].split(" ")[0], clip_file[:-4].split(" ")[2]
            # file_name = clip_file.rsplit(".", 1)[0]
    ...
    # Separate each dataframe for each rep
    ...
    for i in current_pids_list:
        gender_info_list.append(gender_info.get(i, "M"))
...
if __name__ == "__main__":
    ...
    for i, seed_value in enumerate(seed_values):
        ...
        # Creating stats for only one seed value to avoid excessive computation
...
DROP_PARTS=Neck,Nose,LEye,REye,LEar,REar,LBigToe,LSmallToe,LHeel,RBigToe,RSmallToe,RHeel,RKnee,RAnkle,LKnee,LAnkle,MidHip

Neck_prob,Nose_prob,LEye_prob,REye_prob,LEar_prob,REar_prob,LBigToe_prob,LSmallToe_prob,LHeel_prob,RBigToe_prob,RSmallToe_prob,RHeel_prob,RKnee_prob,RAnkle_prob,LKnee_prob,LAnkle_prob,MidHip_prob

Neck_X,Nose_X,LEye_X,REye_X,LEar_X,REar_X,LBigToe_X,LSmallToe_X,LHeel_X,RBigToe_X,RSmallToe_X,RHeel_X,RKnee_X,RAnkle_X,LKnee_X,LAnkle_X,MidHip_X,Neck_Y,Nose_Y,LEye_Y,REye_Y,LEar_Y,REar_Y,LBigToe_Y,LSmallToe_Y,LHeel_Y,RBigToe_Y,RSmallToe_Y,RHeel_Y,RKnee_Y,RAnkle_Y,LKnee_Y,LAnkle_Y,MidHip_Y
Neck_X,Nose_X,LEye_X,REye_X,LEar_X,REar_X,LHeel_X,RHeel_X,RKnee_X,RAnkle_X,LKnee_X,LAnkle_X,MidHip_X,Neck_Y,Nose_Y,LEye_Y,REye_Y,LEar_Y,REar_Y,LHeel_Y,RHeel_Y,RKnee_Y,RAnkle_Y,LKnee_Y,LAnkle_Y,MidHip_Y,RShoulder_X,LShoulder_X,RShoulder_Y,LShoulder_Y,RHip_X,LHip_X,RHip_Y,LHip_Y
Neck_X,Nose_X,LEye_X,REye_X,LEar_X,REar_X,LBigToe_X,LSmallToe_X,LHeel_X,RBigToe_X,RSmallToe_X,RHeel_X,RKnee_X,RAnkle_X,LKnee_X,LAnkle_X,MidHip_X,Neck_Y,Nose_Y,LEye_Y,REye_Y,LEar_Y,REar_Y,LBigToe_Y,LSmallToe_Y,LHeel_Y,RBigToe_Y,RSmallToe_Y,RHeel_Y,RKnee_Y,RAnkle_Y,LKnee_Y,LAnkle_Y,MidHip_Y,RHip_X,LHip_X,RHip_Y,LHip_Y
Neck_X,Nose_X,LEye_X,REye_X,LEar_X,REar_X,LBigToe_X,LSmallToe_X,LHeel_X,RBigToe_X,RSmallToe_X,RHeel_X,RKnee_X,RAnkle_X,LKnee_X,LAnkle_X,MidHip_X,Neck_Y,Nose_Y,LEye_Y,REye_Y,LEar_Y,REar_Y,LBigToe_Y,LSmallToe_Y,LHeel_Y,RBigToe_Y,RSmallToe_Y,RHeel_Y,RKnee_Y,RAnkle_Y,LKnee_Y,LAnkle_Y,MidHip_Y,RHip_X,LHip_X,RHip_Y,LHip_Y,RShoulder_X,LShoulder_X,RShoulder_Y,LShoulder_Y
