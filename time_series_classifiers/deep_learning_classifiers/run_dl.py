...
def create_classifier(classifier_name, input_shape, nb_classes, output_directory, exercise, verbose=False):
    ...
    #     from deep_learning_classifiers.classifiers import mlp
...
if __name__ == "__main__":
    ...
    logger.info("Fitting {} on {}: ".format(classifier_type, "MulticlassSplit"))
    for seed_value in seed_values:
        logger.info("Seed value: {}".format(seed_value))
        data_path = os.path.join(input_data_path, seed_value, "MulticlassSplit")
        weights_directory = os.path.join(output_path, 'dl_weights', exercise, classifier_type, "MulticlassSplit",
                                         seed_value)
        create_directory_if_not_exists(weights_directory)
        datasets_dict = read_dataset(data_path, "MulticlassSplit")
        fit_classifier(classifier_type, datasets_dict, "MulticlassSplit", exercise)
